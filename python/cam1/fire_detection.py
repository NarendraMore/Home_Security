...
import time as t
import datetime
from cam1.database_connection import fire_database
...
RESIZE_WIDTH = 320
RESIZE_HEIGHT = 320
vehicle_labels = {1: 'Fire'}

# Model Paths
PATH_TO_MODEL_DIR = os.path.join(current_dir, "models", "fire_320_320_model","model_2")
...
def create_folder_according_date(base_folder, cameraname):
    ...
    Fire_folder_path = os.path.join(base_folder, model_folder, current_date, cameraname, image_time,image_folder)
    Fire_video_folder_path = os.path.join(base_folder, model_folder, current_date, cameraname,image_time ,video_folder)
    
    os.makedirs(Fire_folder_path, exist_ok=True)
    os.makedirs(Fire_video_folder_path, exist_ok=True)

    return Fire_folder_path, date_for_image, time_for_image, Fire_video_folder_path


def fire_get_frame(frame, cameraname, camlocation, date, time, camip, serverip, fps):
        ...
        if score >= MIN_CONF_THRESH:
            ...
            if last_event_time is None or (current_time - last_event_time) > event_cooldown:
                ...
                if image_saved:
                    ...
                    fourcc = cv2.VideoWriter_fourcc(*'H246')#mp4v
                    ...
                    fire_database(date, time, event, now, cameraname, camlocation, image_path, camip, serverip, videopath=video_path)
